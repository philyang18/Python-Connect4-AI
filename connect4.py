...
def score_position(board, piece):
    ...
    for r in range(ROW_COUNT):
        ...
        for c in range(COLUMN_COUNT - 3):
            ...
            # if Player is first, then if r % 2 is 1 then add more points to that
            if FIRST_PLAYER == piece:
            	if r % 2 == 1:
            		score += 5
            else:
            	if r % 2 == 0:
...
def animate_move(board, row, col, piece):
    xposition = int(col*SQUARE_SIZE + SQUARE_SIZE/2)
    yposition = int(SQUARE_SIZE/2)
    COLOR = RED
    if piece == AI_PIECE:
        COLOR = YELLOW
    yspeed = 2
    while yposition < (height-row*SQUARE_SIZE-SQUARE_SIZE/2):
        yspeed += .5
        yposition += yspeed
...
while not game_over:
    ...
    if turn == AI and not game_over:
        ...
        if is_valid_location(board, col):
            ...
            if winning_move(board, AI_PIECE):
                label = myfont.render("Sorry you lose.", 1, YELLOW)
                screen.blit(label, (40,10))
                game_over = True
